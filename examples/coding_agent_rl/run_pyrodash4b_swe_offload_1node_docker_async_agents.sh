...
# Soft-insert：前期多 route 灌 tag；TRAJ=0.50 留 ~15–20pp 给自然；END 留底
OFFLOAD_ROUTE_PROB=0.30
OFFLOAD_ROUTE_PROB_END=0.1
OFFLOAD_ROUTE_ANNEAL_STEPS=80
OFFLOAD_FORCE_TAG_TRAJ_FRAC=0.50    # 总 offload 帽（route + 自然）
OFFLOAD_ROUTE_STEP_BIAS=100
OFFLOAD_ROUTE_SUPPRESS_NATURAL=0    # 先不压自然；toff 飙高再开
OFFLOAD_ROUTE_SUPPRESS_BIAS=100
OFFLOAD_REWARD_MODE=help_seeking
OFFLOAD_SEEK_ONLY_WHEN_ALL_WRONG=1  # 有人能 solo 解 → 不给瞎求助 α
# natural-only：nat_t EMA≥2.5% 再 latch（历史峰值 ~1–4%，10% 不现实）
OFFLOAD_SEEK_NATURAL_ONLY_AFTER=0
OFFLOAD_SEEK_NATURAL_ONLY_MIN_NAT_T=0.03
# α 略降护 solve；λ/unique 退火 → 后期抬自己做对
OFFLOAD_REWARD_ANNEAL_STEPS=80
OFFLOAD_SEEK_ALPHA=0.12             # 固定：卡住要求助（低于旧 0.20）
OFFLOAD_NO_SEEK_PENALTY=0.18        # 固定：全挂且无 credit 求助 → 罚
OFFLOAD_EFFICIENCY_LAMBDA=0.12      # early: 少罚 cost
OFFLOAD_EFFICIENCY_LAMBDA_END=0.22  # late: 做对时重 cost
OFFLOAD_UNIQUE_SOLVER_BONUS=0.15    # early
...
export OFFLOAD_EFFICIENCY_LAMBDA="${OFFLOAD_EFFICIENCY_LAMBDA:-0.12}"
...
export OFFLOAD_SEEK_NATURAL_ONLY_MIN_NAT_T="${OFFLOAD_SEEK_NATURAL_ONLY_MIN_NAT_T:-0.025}"
unset OFFLOAD_SEEK_NATURAL_ONLY_MIN_NAT_S 2>/dev/null || true
export OFFLOAD_REWARD_ANNEAL_STEPS="${OFFLOAD_REWARD_ANNEAL_STEPS:-${OFFLOAD_ROUTE_ANNEAL_STEPS:-120}}"
export OFFLOAD_SEEK_ALPHA="${OFFLOAD_SEEK_ALPHA:-0.12}"
...
# Target: higher early route under TRAJ=0.50; anneal to floor (not 0);
# latch seek credit at modest nat_t; suppress off until density blows up.
export OFFLOAD_ROUTE_PROB="${OFFLOAD_ROUTE_PROB:-${OFFLOAD_FORCE_TAG_PROB:-0.30}}"
export OFFLOAD_FORCE_TAG_PROB="${OFFLOAD_FORCE_TAG_PROB:-${OFFLOAD_ROUTE_PROB}}"
export OFFLOAD_ROUTE_PROB_END="${OFFLOAD_ROUTE_PROB_END:-0.15}"
export OFFLOAD_ROUTE_ANNEAL_STEPS="${OFFLOAD_ROUTE_ANNEAL_STEPS:-120}"
...
export OFFLOAD_ROUTE_SUPPRESS_NATURAL="${OFFLOAD_ROUTE_SUPPRESS_NATURAL:-0}"
export OFFLOAD_ROUTE_SUPPRESS_BIAS="${OFFLOAD_ROUTE_SUPPRESS_BIAS:-${OFFLOAD_ROUTE_STEP_BIAS}}"
export OFFLOAD_FORCE_TAG_TRAJ_FRAC="${OFFLOAD_FORCE_TAG_TRAJ_FRAC:-0.50}"
...
echo "  OFFLOAD_SEEK_ALPHA=${OFFLOAD_SEEK_ALPHA} (flat) NATURAL_ONLY_AFTER=${OFFLOAD_SEEK_NATURAL_ONLY_AFTER:-0} MIN_NAT_T=${OFFLOAD_SEEK_NATURAL_ONLY_MIN_NAT_T:-0.025}"
...
echo "  OFFLOAD_ROUTE_PROB=${OFFLOAD_ROUTE_PROB:-0.30}→${OFFLOAD_ROUTE_PROB_END:-0.15} over ${OFFLOAD_ROUTE_ANNEAL_STEPS:-120} (FORCE_TAG_PROB=${OFFLOAD_FORCE_TAG_PROB:-0.30}) TRAJ_FRAC=${OFFLOAD_FORCE_TAG_TRAJ_FRAC:-0.50} SUPPRESS_NATURAL=${OFFLOAD_ROUTE_SUPPRESS_NATURAL:-0} MIN_TURN=${OFFLOAD_FORCE_TAG_MIN_TURN:-0} MAX=${OFFLOAD_FORCE_TAG_MAX:-0} N=${OFFLOAD_FORCE_TAG_N:-random}"
