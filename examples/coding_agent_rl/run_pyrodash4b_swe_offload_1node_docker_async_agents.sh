...
OFFLOAD_ROUTE_PROB=0.35             # 前期多 route：先学会 tag
OFFLOAD_ROUTE_PROB_END=0.0          # 后期关掉 route，逼自然
OFFLOAD_ROUTE_ANNEAL_STEPS=40       # 线性降到 0
OFFLOAD_FORCE_TAG_TRAJ_FRAC=0.35    # 总 offload ≈35%（route + 自然）
OFFLOAD_ROUTE_STEP_BIAS=100
OFFLOAD_ROUTE_SUPPRESS_NATURAL=1    # 到 35% 帽后压自然，防飙高伤 solve
OFFLOAD_ROUTE_SUPPRESS_BIAS=100
OFFLOAD_REWARD_MODE=help_seeking
OFFLOAD_SEEK_ONLY_WHEN_ALL_WRONG=1  # 有人能 solo 解 → 不给瞎求助 α
# ≥25：seek α / no_seek 只认自然（route 只曝光 tag，不拿学分）
OFFLOAD_SEEK_NATURAL_ONLY_AFTER=25
# 自然不另做概率退火；α/NO_SEEK 固定。λ/unique 仍退火 → 后期抬 solve
OFFLOAD_REWARD_ANNEAL_STEPS=40
OFFLOAD_SEEK_ALPHA=0.20             # 固定：卡住要求助
OFFLOAD_NO_SEEK_PENALTY=0.18        # 固定：全挂且无自然求助 → 罚
...
# help_seeking + SEEK_ONLY_WHEN_ALL_WRONG. Natural: no separate prob anneal —
# switch at NATURAL_ONLY_AFTER; route anneals to 0. λ/unique still anneal for solve.
export OFFLOAD_REWARD_MODE=help_seeking
export OFFLOAD_SEEK_ONLY_WHEN_ALL_WRONG=1
export OFFLOAD_SEEK_NATURAL_ONLY_AFTER="${OFFLOAD_SEEK_NATURAL_ONLY_AFTER:-25}"
export OFFLOAD_REWARD_ANNEAL_STEPS="${OFFLOAD_REWARD_ANNEAL_STEPS:-${OFFLOAD_ROUTE_ANNEAL_STEPS:-40}}"
...
# Target: early high ROUTE bootstraps tags; after SEEK_NATURAL_ONLY_AFTER only
# natural seeks earn α / avoid NO_SEEK. ROUTE anneals to ~0 so free decode must emit.
export OFFLOAD_ROUTE_PROB="${OFFLOAD_ROUTE_PROB:-${OFFLOAD_FORCE_TAG_PROB:-0.35}}"
export OFFLOAD_FORCE_TAG_PROB="${OFFLOAD_FORCE_TAG_PROB:-${OFFLOAD_ROUTE_PROB}}"
export OFFLOAD_ROUTE_PROB_END="${OFFLOAD_ROUTE_PROB_END:-0.0}"
export OFFLOAD_ROUTE_ANNEAL_STEPS="${OFFLOAD_ROUTE_ANNEAL_STEPS:-40}"
...
echo "  OFFLOAD_SEEK_ALPHA=${OFFLOAD_SEEK_ALPHA} (flat) NATURAL_ONLY_AFTER=${OFFLOAD_SEEK_NATURAL_ONLY_AFTER:-25}"
...
echo "  OFFLOAD_ROUTE_PROB=${OFFLOAD_ROUTE_PROB:-0.35}→${OFFLOAD_ROUTE_PROB_END:-0.0} over ${OFFLOAD_ROUTE_ANNEAL_STEPS:-40} (FORCE_TAG_PROB=${OFFLOAD_FORCE_TAG_PROB:-0.35}) TRAJ_FRAC=${OFFLOAD_FORCE_TAG_TRAJ_FRAC:-0.35} SUPPRESS_NATURAL=${OFFLOAD_ROUTE_SUPPRESS_NATURAL:-1} MIN_TURN=${OFFLOAD_FORCE_TAG_MIN_TURN:-0} MAX=${OFFLOAD_FORCE_TAG_MAX:-0} N=${OFFLOAD_FORCE_TAG_N:-random}"
