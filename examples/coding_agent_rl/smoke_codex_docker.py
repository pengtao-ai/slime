#!/usr/bin/env python3
"""Lightweight Codex ↔ OpenAIAdapter ↔ local SGLang smoke (no Ray / no train.sh).

Prereq — start SGLang, then build the offline Codex tarball once::

    python examples/coding_agent_rl/build_codex_local_tarball.py
    python examples/coding_agent_rl/smoke_codex_docker.py
"""
...
logger = logging.getLogger("smoke_codex")

DEFAULT_JSONL = smoke.DEFAULT_JSONL
DEFAULT_NODE = smoke.DEFAULT_NODE
DEFAULT_CODEX = _EXAMPLE_DIR / "tarballs" / "openai-codex-local.tgz"
...
async def run_smoke(args: argparse.Namespace) -> None:
    ...
    handle = run_app_in_thread(
        ...
        host=args.bind_host,
        port=args.bind_port,
        thread_name="smoke-codex-adapter",
        runner_kwargs={"handler_cancellation": True, "access_log_class": FilteredAccessLogger},
    )
    print(
        f"[smoke] openai adapter listening {args.bind_host}:{handle.port}  sglang={args.sglang_url}",
        flush=True,
    )
    ...
    session_id = f"smoke-codex-{secrets.token_hex(8)}"
    ...
    try:
        ...
        async with DockerSandbox(image) as sb:
            ...
            print(
                f"[smoke] adapter_url will be {adapter_url} "
                f"(codex base_url={adapter_url}/v1, wire_api=chat)",
                flush=True,
            )

            harness = CodexHarness()
            print("[smoke] install_cli (node + codex)...", flush=True)
            ...
            print(
                f"[smoke] codex.run budget={args.time_budget}s prompt={args.prompt[:80]!r}...",
                flush=True,
            )
            ...
            _, diag, _ = await sb.exec(
                ...
                "echo '=== codex config ==='; "
                "cat /home/agent/.codex/config.toml 2>/dev/null || true; "
        ...
        segments = await adapter.finish_session(
            ...
            base_sample=sample,
            reward=float(reward),
            extra_metadata={"instance_id": instance_id, "smoke": True, "agent": "codex"},
        )
    finally:
    ...
    if not segments:
        raise SystemExit(
            "FAIL: adapter_session_empty — codex never completed a turn "
            "through OpenAIAdapter→SGLang. Check diag above."
        )
...
def main() -> None:
    ...
    p.add_argument(
        "--codex-tarball",
        type=Path,
        default=Path(os.environ.get("SLIME_AGENT_CODEX_TARBALL", DEFAULT_CODEX)),
    )
    ...
    p.add_argument("--bind-port", type=int, default=int(os.environ.get("ADAPTER_PORT", "18094")))
    ...
    p.add_argument("--prompt", default=os.environ.get("SWE_CODEX_PROMPT", DEFAULT_PROMPT))
    p.add_argument("--require-exit-zero", action="store_true")
    args = p.parse_args()

    smoke._require_file(args.node_tarball, "SLIME_AGENT_NODE_TARBALL")
    smoke._require_file(args.codex_tarball, "SLIME_AGENT_CODEX_TARBALL")
