#!/usr/bin/env python3
"""Lightweight OpenCode ↔ AnthropicAdapter ↔ local SGLang smoke (no Ray / no train.sh).

Mirrors ``smoke_claude_code_docker.py`` but swaps in ``OpenCodeHarness``.
...
Build the offline OpenCode tarball once::

    python examples/coding_agent_rl/build_opencode_local_tarball.py

Then::

    python examples/coding_agent_rl/smoke_opencode_docker.py
"""
...
from slime.agent.harness import OpenCodeHarness
from slime.agent.sandbox import DockerSandbox, ensure_agent_user
from slime.utils.processing_utils import load_tokenizer

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger("smoke_opencode")

DEFAULT_JSONL = smoke.DEFAULT_JSONL
DEFAULT_NODE = smoke.DEFAULT_NODE
DEFAULT_OPENCODE = _EXAMPLE_DIR / "tarballs" / "opencode-ai-local-linux-x64.tgz"
...
async def run_smoke(args: argparse.Namespace) -> None:
    ...
    session_id = f"smoke-oc-{secrets.token_hex(8)}"
    ...
    try:
        ...
        async with DockerSandbox(image) as sb:
            ...
            print(f"[smoke] adapter_url will be {adapter_url} (OpenCode baseURL=+ /v1)", flush=True)

            harness = OpenCodeHarness()
            print("[smoke] install_cli (node + opencode)...", flush=True)
            ...
            print(
                f"[smoke] opencode.run budget={args.time_budget}s prompt={args.prompt[:80]!r}...",
                flush=True,
            )
            ...
            _, diag, _ = await sb.exec(
                ...
                "cat /home/agent/.config/opencode/opencode.json 2>/dev/null || true; "
                "echo '=== opencode.log tail ==='; "
                "tail -c 3000 /home/agent/.local/share/opencode/log/opencode.log 2>/dev/null || true",
                timeout=30,
            )
        ...
        segments = await adapter.finish_session(
            ...
            base_sample=sample,
            reward=float(reward),
            extra_metadata={"instance_id": instance_id, "smoke": True, "agent": "opencode"},
        )
    finally:
    ...
    if not segments:
        raise SystemExit(
            "FAIL: adapter_session_empty — OpenCode never completed a turn "
            "through AnthropicAdapter→SGLang. Check diag above "
            "(baseURL /v1? adapter unreachable? SGLang /generate errors?)."
        )
...
def main() -> None:
    ...
    p.add_argument(
        "--opencode-tarball",
        type=Path,
        default=Path(os.environ.get("SLIME_AGENT_OPENCODE_TARBALL", DEFAULT_OPENCODE)),
    )
    ...
    p.add_argument("--prompt", default=os.environ.get("SWE_OPENCODE_PROMPT", DEFAULT_PROMPT))
    ...
    smoke._require_file(args.opencode_tarball, "SLIME_AGENT_OPENCODE_TARBALL")
