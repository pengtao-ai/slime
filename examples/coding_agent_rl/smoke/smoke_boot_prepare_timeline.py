...
Run (micromamba slime)::
    ...
    micromamba run -n slime python examples/coding_agent_rl/smoke/smoke_boot_prepare_timeline.py
"""
...
os.environ.setdefault(
    ...
    str(_EXAMPLE_DIR / "tarballs" / "node-v22.20.0-linux-x64.tar.xz"),
)
os.environ.setdefault(
    "SLIME_AGENT_CC_TARBALL",
    str(_EXAMPLE_DIR / "tarballs" / "anthropic-ai-claude-code-local-linux-x64.tgz"),
)
...
def main() -> None:
    ...
    p.add_argument(
        "--prompt-data",
        type=Path,
        default=_EXAMPLE_DIR / "data" / "swe_smoke_preliz_docker.jsonl",
    )
