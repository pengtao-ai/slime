#!/usr/bin/env python3
"""Build an offline-installable OpenCode npm tarball for slime sandboxes.

Downloads opencode-ai + opencode-linux-x64-baseline, bakes the native binary
into bin/opencode.exe, strips postinstall/optionalDeps, and writes
opencode-ai-local-linux-x64.tgz (npm install -g compatible).

Baseline binary is preferred over AVX2-only builds for broader sandbox CPUs.
"""
...
VER = "1.18.14"
WRAPPER = f"opencode-ai@{VER}"
BINARY_PKG = f"opencode-linux-x64-baseline@{VER}"
...
def main() -> None:
    ...
    workdir = Path(tempfile.mkdtemp(prefix="opencode-local-"))
    print("workdir", workdir)

    wrapper_cached = OUT / f"opencode-ai-{VER}.tgz"
    ...
    candidates = [p for p in files if p.name == "opencode"]
    if not candidates:
        candidates = [
            p
            for p in files
            if "opencode" in p.name.lower() and p.suffix not in {".md", ".json", ".txt"}
        ]
    if not candidates:
        raise RuntimeError(
            f"no opencode binary in {bpkg}; sample={[str(p.relative_to(bpkg)) for p in files[:20]]}"
        )
    binary = max(candidates, key=lambda p: p.stat().st_size)
    print(f"using binary {binary.relative_to(bpkg)} ({binary.stat().st_size} bytes)")

    dest = pkg / "bin" / "opencode.exe"
    ...
    out_tgz = OUT / "opencode-ai-local-linux-x64.tgz"
