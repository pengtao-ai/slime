...
if [[ "${SLIME_AGENT_OFFLOAD:-}" == "1" ]]; then
  echo "---- offload / help_seeking / SFT (also → run.log + ${RUN_ROOT}/offload_config.json) ----"
  echo "  OFFLOAD_REWARD_MODE=${OFFLOAD_REWARD_MODE:-cost_aware}"
  echo "  OFFLOAD_EFFICIENCY_LAMBDA=${OFFLOAD_EFFICIENCY_LAMBDA:-0.6}"
  echo "  OFFLOAD_SEEK_ONLY_WHEN_ALL_WRONG=${OFFLOAD_SEEK_ONLY_WHEN_ALL_WRONG:-}"
  echo "  OFFLOAD_SEEK_ALPHA=${OFFLOAD_SEEK_ALPHA:-0.1}"
  echo "  OFFLOAD_SEEK_EMPTY_SCALE=${OFFLOAD_SEEK_EMPTY_SCALE:-0.5}"
  echo "  OFFLOAD_UNIQUE_SOLVER_BONUS=${OFFLOAD_UNIQUE_SOLVER_BONUS:-0.15}"
...
# Compact filter + help_seeking α unless a sibling solved without offload.
...
# Multi-agent harness CLIs (mixed_*_agents.jsonl); same defaults as infer_cc_offload_traj.py.
export SLIME_AGENT_CODEX_TARBALL="${SLIME_AGENT_CODEX_TARBALL:-${SCRIPT_DIR}/tarballs/openai-codex-local.tgz}"
export SLIME_AGENT_PI_TARBALL="${SLIME_AGENT_PI_TARBALL:-${SCRIPT_DIR}/tarballs/pi-coding-agent-local.tgz}"
export SLIME_AGENT_OPENCODE_TARBALL="${SLIME_AGENT_OPENCODE_TARBALL:-${SCRIPT_DIR}/tarballs/opencode-ai-local-linux-x64.tgz}"
export SLIME_AGENT_MINISWE_WHEEL="${SLIME_AGENT_MINISWE_WHEEL:-${SCRIPT_DIR}/tarballs/miniswe-wheels}"
...
keys = (
    ...
    "SLIME_AGENT_CODEX_TARBALL", "SLIME_AGENT_PI_TARBALL",
