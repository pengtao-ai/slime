...
SLM step, colored by the git diff captured at that turn. Same color = same
non-empty git diff across sibling trajectories.
...
Usage:
  PYTHONPATH=. python examples/coding_agent_rl/render_gigpo_instance_grid_by_git_diff_html.py \
    ...
    --out runs/.../gigpo_instance_grid_by_git_diff_sybil_pr296.html
"""
            ...
            rows.append(
                {
                    ...
                    "diff_key": diff_key,
                    "diff_summary": _summarize_diff(diff_key),
                    "is_init": t["is_init"],
                    "prev_tool_call": (
...
def assign_group_ids(rows: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for r in rows:
        key = r["diff_key"]
        by_diff[key].append(r)
    diff_to_gid: dict[str, int] = {}
    gid = 0
    for diff_text, members in sorted(by_diff.items(), key=lambda kv: (-len(kv[1]), kv[0])):
        if len(members) >= 2:
            diff_to_gid[diff_text] = gid
            gid += 1
    return diff_to_gid
...
def _step_cell(m: dict[str, Any] | None, diff_to_gid: dict[str, int]) -> str:
    if m is None:
        return "<td class='step empty'></td>"
    gid = diff_to_gid.get(m["diff_key"], -1)
    color = GROUP_COLORS[gid % len(GROUP_COLORS)] if gid >= 0 else "#3a3f47"
    gid_label = f"D{gid}" if gid >= 0 else "·"
...
def render_html(
    *,
    ...
    for _, steps in trajs:
        for s in steps:
            key = s["diff_key"]
            by_diff[key].append(s)

    legend_parts = []
    for diff_text, members in sorted(by_diff.items(), key=lambda kv: -len(kv[1])):
        if len(members) < 2:
            continue
        gid = diff_to_gid[diff_text]
        color = GROUP_COLORS[gid % len(GROUP_COLORS)]
        legend_parts.append(
            f"<span class='chip' style='background:{color}' title='{html.escape(diff_text[:400])}'>"
            f"D{gid} {html.escape(_summarize_diff(diff_text))} ×{len(members)}</span>"
        )
    ...
    for t in range(max_turn + 1):
        ...
        for idx in sample_indices:
            ...
            if m is not None:
                any_cell = True
            cells.append(_step_cell(m, diff_to_gid))
        if not any_cell:
            continue
    ...
    return (
        ...
        f"<title>GiGPO git diff {html.escape(instance_id)} · rollout {rollout_id}</title>"
        f"<style>{CSS}</style></head><body>\n"
        "<header>\n"
        f"  <h1>GiGPO git diff 入组情况 — {html.escape(instance_id)}（{len(trajs)} 条 traj 并排）</h1>\n"
        f"  <div class='sub'>rollout {rollout_id} · {rollout_path} · {len(trajs)} 条 traj · "
        f"成组 step {n_matched_steps}/{n_total_steps} ({match_pct:.0f}%) · 同色 = 同 git diff</div>\n"
        f"  <div class='legend'>{legend}</div>\n"
        "</header>\n"
        "<main>\n"
        "  <p style='color:#8b949e;font-size:10px;margin:4px 0 8px'>"
        "每行 = 同一 turn 索引；每列 = 一条 sibling traj。同色块 = 同 git diff；仅出现一次的组仍显示为灰。鼠标悬停看完整 diff。</p>\n"
        f"{table}\n"
        "</main>\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    diff_to_gid = assign_group_ids(rows)
    ...
    n_matched = sum(1 for r in rows if r["diff_key"] in diff_to_gid)
    html_str = render_html(
        instance_id=args.instance,
        rollout_id=rollout_id,
        rollout_path=str(args.rollout),
        trajs=trajs,
        diff_to_gid=diff_to_gid,
    ...
    print(
        ...
        f"{n_matched} matched ({n_matched / max(len(rows), 1) * 100:.0f}%), md5={digest}"
    )
    return 0
