#!/usr/bin/env python3
"""Lightweight mini-swe-agent ↔ AnthropicAdapter ↔ local SGLang smoke.

Prereq — start SGLang, then download wheels once::

    python examples/coding_agent_rl/build_miniswe_wheels.py
    python examples/coding_agent_rl/smoke_miniswe_docker.py
"""
...
from slime.agent.harness import MiniSweHarness
from slime.agent.sandbox import DockerSandbox, ensure_agent_user
from slime.utils.processing_utils import load_tokenizer

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
logger = logging.getLogger("smoke_miniswe")

DEFAULT_JSONL = smoke.DEFAULT_JSONL
DEFAULT_WHEELS = _EXAMPLE_DIR / "tarballs" / "miniswe-wheels"
...
async def run_smoke(args: argparse.Namespace) -> None:
    ...
    handle = run_app_in_thread(
        ...
        host=args.bind_host,
        port=args.bind_port,
        thread_name="smoke-miniswe-adapter",
    ...
    session_id = f"smoke-ms-{secrets.token_hex(8)}"
    ...
    try:
        ...
        async with DockerSandbox(image) as sb:
            ...
            print(f"[smoke] adapter_url will be {adapter_url} (litellm anthropic api_base)", flush=True)

            harness = MiniSweHarness()
            print("[smoke] install_cli (pip mini-swe-agent)...", flush=True)
            ...
            print(
                f"[smoke] mini.run budget={args.time_budget}s prompt={args.prompt[:80]!r}...",
                flush=True,
            )
            ...
            _, diag, _ = await sb.exec(
                ...
                "echo '=== mini config ==='; "
                "cat /home/agent/.config/mini-swe-agent/slime.yaml 2>/dev/null || true; "
        ...
        segments = await adapter.finish_session(
            ...
            base_sample=sample,
            reward=float(reward),
            extra_metadata={"instance_id": instance_id, "smoke": True, "agent": "miniswe"},
        )
    finally:
    ...
    if not segments:
        raise SystemExit(
            "FAIL: adapter_session_empty — mini never completed a turn "
...
def main() -> None:
    ...
    p.add_argument(
        "--miniswe-wheel",
        type=Path,
        default=Path(os.environ.get("SLIME_AGENT_MINISWE_WHEEL", DEFAULT_WHEELS)),
    )
    ...
    p.add_argument("--bind-port", type=int, default=int(os.environ.get("ADAPTER_PORT", "18093")))
    ...
    p.add_argument("--prompt", default=os.environ.get("SWE_MINISWE_PROMPT", DEFAULT_PROMPT))
    p.add_argument("--require-exit-zero", action="store_true")
    args = p.parse_args()

    if not args.miniswe_wheel.exists():
        raise SystemExit(
            f"missing SLIME_AGENT_MINISWE_WHEEL={args.miniswe_wheel}; "
            "run: python examples/coding_agent_rl/build_miniswe_wheels.py"
        )
