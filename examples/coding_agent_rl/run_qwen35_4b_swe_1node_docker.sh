...
# Match training --rollout-max-response-len default (32768). Without this, Claude
# Code may abort with "exceeded the 32000 output token maximum".
if [[ -z "${SLIME_AGENT_CC_EXTRA_ENVS:-}" ]]; then
  export SLIME_AGENT_CC_EXTRA_ENVS='{"CLAUDE_CODE_MAX_OUTPUT_TOKENS":"32768"}'
fi

# Default: ScaleSWE 200-prompt train set (Docker images in metadata.image).
# Override PROMPT_DATA to fall back to the single-row smoke jsonl if needed.
export PROMPT_DATA="${PROMPT_DATA:-${SCRIPT_DIR}/data/swe_train_scaleswe_200.jsonl}"
export EXP_TAG="${EXP_TAG:-agent_only_qwen35_4b_docker_scaleswe200}"

# Fan-out: each prompt boots an agent sandbox (+ a later eval sandbox).
# Keep concurrent sandboxes modest on docker-rt; raise if your host is stable.
# With batch=2 and n_samples=4 → 8 agent containers per rollout step.
...
# ~200 prompts / batch_size ≈ 100 steps for one pass; override as needed.
...
echo "  SLIME_AGENT_CC_EXTRA_ENVS=${SLIME_AGENT_CC_EXTRA_ENVS}"
