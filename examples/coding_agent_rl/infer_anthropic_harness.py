"""Shared Anthropic-harness infer loop (OpenCode / pi / mini-swe).

Callers set agent-specific env vars and pass harness + mode name.
"""
...
import swe
from traj_dump import TrajRecordingAnthropicAdapter, is_tool_turn, load_rows, save_outputs

logger = logging.getLogger("infer_anthropic_harness")


async def run_one(
    args: argparse.Namespace,
    adapter: TrajRecordingAnthropicAdapter,
    handle: Any,
    row: dict[str, Any],
    idx: int,
    out_root: Path,
...
async def main_async(
    ...
    *,
    ...
    print(
        f"[infer] adapter {args.bind_host}:{handle.port} sglang={args.sglang_url} "
...
def add_common_infer_args(p: argparse.ArgumentParser, *, default_bind_port: int) -> None:
    ...
    p.add_argument("--hf-checkpoint", default="/workspace/models/Qwen/Qwen3.5-4B")
    ...
    p.add_argument("--time-budget", type=int, default=1800)
    p.add_argument("--max-context-len", type=int, default=96000)
    p.add_argument("--max-new-tokens", type=int, default=4096)
    p.add_argument("--temperature", type=float, default=0.7)
    p.add_argument("--prompt", default="")
    p.add_argument("--pull", action="store_true")
    # silence unused in helper signature documentation
    _ = ex
