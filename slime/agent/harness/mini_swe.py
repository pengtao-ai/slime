...
class MiniSweHarness(BaseHarness):
    ...
    async def install_cli(self, sb: Sandbox) -> None:
        ...
        # Pre-baked images already ship mini under /usr/local; skip wheel upload.
        ec, _, _ = await sb.exec(
            "test -x /usr/local/bin/mini && /usr/local/bin/mini --help >/dev/null",
            user="root",
            check=False,
            timeout=60,
        )
        ...
        if wheel.is_dir():
            ...
            install_cmd = (
                ...
                # which(mini) may already be /usr/local/bin/mini (pip --prefix); ln
                # same-path fails with "are the same file" under set -e.
